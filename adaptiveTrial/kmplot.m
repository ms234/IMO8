...
if flag %if this function was not called by LOGRANK function
    ...
    hold on
    %S2=stairs(t1(1:lowcend),lowc(1:lowcend),'k--','LineWidth',0); %lower confidence interval curve
    ...
    if mt
        if isempty(S4)
%           ll= legend([S1 S2 S3],'Data',str1,str2)
        else
%           ll= legend([S1 S2 S3 S4],'Data',str1,str2,'Censored')
        end
    else
        if isempty(S4)
 %           ll=legend([S1 S2],'Data',str1)
        else
 %           ll=legend([S1 S2 S4],'Data',str1,'Censored')
        end
    end
end
